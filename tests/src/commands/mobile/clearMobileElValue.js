...
import ClearMobileElValue from "../../../../lib/commands/mobile/clearMobileElValue";
...
const immutableClientMock = {
  ...
  api: {
    elementIdClear: (el, callback) => { callback(result) }
  },
  assertion: function (result, actual, expected, message, abortonfail) { }
};


describe("ClearMobileElValue", () => {
  let clearMobileElValue = null;
  let clientMock = null;

  beforeEach(() => {
    clientMock = _.cloneDeep(immutableClientMock);
    clearMobileElValue = new ClearMobileElValue(clientMock);

    result.status = 0;
  });

  it("Initialization", () => {
    expect(clearMobileElValue.cmd).to.equal("clearMobileElValue");
  });
  ...
  it("Pass", (done) => {
    ...
    clearMobileElValue = new ClearMobileElValue(clientMock);
    clearMobileElValue.command("accessibility id", "Search");
  });
  ...
  it("Fail in protocol call", (done) => {
    ...
    clearMobileElValue = new ClearMobileElValue(clientMock);
    clearMobileElValue.command("accessibility id", "Search");
    clearMobileElValue.startTime = Date.now() - 60 * 1000;
  });

  it("Fail in selenium call", (done) => {
    clientMock.api.elementIdClear = (el, callback) => {
      callback({
        status: -1,
